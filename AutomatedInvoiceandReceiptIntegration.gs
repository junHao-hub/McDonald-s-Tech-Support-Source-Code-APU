...
function CreateInvoice() {
  ...
  let datalist=[documentid,clientname,date,list[0],list[1]];
...
function MakeTable(){
  ...
  //Ask how many rows
  let ui=DocumentApp.getUi();
  let counter=QuestionAndCheck("How many rows would you like to have? ",false);
  ...
  for (let i=1;i<counter+1;i++){
    ...
    for (let j=1;j<3;j++){
      ...
      response=QuestionAndCheck("What is your "+questions[j]+" for product "+ name +"?",floatcheck); 
  ...
  let discount=ui.prompt("How much discount is given?(%)").getResponseText()-'0';
  row=newtable.appendTableRow();
  for (let i=0;i<3;i++){
    row.appendTableCell();
  }
  row.appendTableCell("DISCOUNT: "+discount+"%");
  row.appendTableCell('('+(productsum*(discount)/100).toFixed(2)+')');
  newtable.getRow(counter+1).getCell(3).getChild(0).asParagraph().setAttributes(bodyStyle);
  newtable.getRow(counter+1).getCell(4).getChild(0).asParagraph().setAttributes(bodyStyle);
  row=newtable.appendTableRow();
  for (let i=0;i<3;i++){
    row.appendTableCell();
  }
  row.appendTableCell("GRAND TOTAL");
  row.appendTableCell((productsum*(100-'0'-discount)/100).toFixed(2));
  for (let i=3;i<5;i++){
    newtable.getRow(counter+2).getCell(i).getChild(0).asParagraph().setAttributes(bodyStyle);
  }

  return [(productsum*(100-'0'-discount)/100).toFixed(2),discount];
}
...
function CreateReceipt(){
  ...
  let amount=QuestionAndCheck("What is the Amount(RM) Received?",true);
...
function TransferToInvoiceSheet(datalist){
  ...
  displayindex++;
  //Add row
  sheet.appendRow([displayindex,documentid,client,date,amount+' ('+discount+'% DISCOUNT)',url]);
